...
def parser(subparsers, repo):
    ...
    runrepo_parser.add_argument(
        '-q', '--query', help='Query the status of your running job on the server', action='store_true'
    )
    runrepo_parser.set_defaults(func=main)

def main(args, repo):
    if core.Constants.verbose_conf_dialog(repo.current_branch, "runrepo", args, core.Constants.try_get_upstream(repo, core.pygit2.GIT_BRANCH_LOCAL)):
    ...
    try:
        ...
        if args.abort:
            pprint.msg(Client.abort(server, port, Constants.username))
        if args.query:
            pprint.msg(Client.query(server, port, Constants.username))
        else:
            ...
            try:
                ...
                pprint.err(f"There was a major fault starting the repository: {e}")
        return True
    except Exception as e:
